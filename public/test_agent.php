...
if ($selectedFile && file_exists($directory . $selectedFile)) {
    $agentYaml = file_get_contents($directory . $selectedFile);
}
...
<div class="container mt-5">
    ...
    <ul class="list-group">
      <?php foreach ($files as $file): 
        ...
        <li class="list-group-item d-flex justify-content-between align-items-center">
          ...
          <div>
            <a href="manage_agent.php?file=<?= urlencode($basename) ?>" class="btn btn-sm btn-secondary">Edit</a>
            <a href="test_agent.php?file=<?= urlencode($basename) ?>" class="btn btn-sm btn-success">Test</a>
          </div>
        </li>
      <?php endforeach; ?>
    </ul>
  ...
  <?php if ($selectedFile && $agentYaml): ?>
  <div class="card p-4">
    <h4>Testing Agent: <?= htmlspecialchars($selectedFile) ?></h4>
    <form id="aiTestForm">
      <input type="hidden" name="type" value="agent">
      <input type="hidden" name="yaml" value="<?= htmlspecialchars($agentYaml) ?>">
